...
#data = pd.concat([zero_class, one_class, one_class])
...
rejected_variables = logistic_regression.coef_
selected_vars = data.columns[[x != 0 for x in rejected_variables[0]]]

print("selected variables: ", list(selected_vars))
print("rejected: ", list(set(data.columns) - set(selected_vars)))
...
dsdf = data.columns[[i > 0 for i in ada_boost_tree.feature_importances_]]
print(dsdf)
print(len(dsdf))
